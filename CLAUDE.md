...
# Benchmark against running docling-serve instance
...
- `Dockerfile` — Multi-stage GPU-optimized build (the core of this project)
- `test_docling.py` — Python diagnostics (10-check GPU report) and HTTP benchmark tool (4 task types)
- `benchmark.sh` — Shell script comparing official vs custom image with automated container lifecycle
- `h20_diag.sh` — Self-contained diagnostic for H100/H200 cloud pods (generates test PDF inline)
- `files/attention.pdf` — "Attention Is All You Need" paper used as benchmark input
